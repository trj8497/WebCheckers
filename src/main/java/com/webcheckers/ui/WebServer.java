...
public class WebServer {
    ...
    /**
     * URL to request the modes
     */
    public static final String MODE_URL = "/modes";
    ...
    public static final String GAME_OVER = "gameOverMessage";
    ...
    public static final String AI_GAME = "/startaigame";
    ...
    public void initialize() {
        ...
        get(HOME_URL, new GetHomeRoute(this.playerLobby, templateEngine));

        get(LOGIN_URL, new GetLogInRoute(this.playerLobby, templateEngine));

        post(LOGIN_URL, new PostSignInRoute(this.playerLobby, templateEngine));

        get(GAME_URL, new GetGameRoute(this.playerLobby, templateEngine, gson));

        get(MODE_URL, new GetModesRoute(this.playerLobby, templateEngine));

        post(MODE_URL, new PostModesRoute(this.playerLobby, templateEngine));

        post(SIGN_OUT_URL, new PostSignOutRoute(this.playerLobby, templateEngine));

        post(START_GAME, new PostStartGame(this.playerLobby, templateEngine));
        //
        post(RESIGN_GAME, new PostResignGameRoute(this.gson));

        post(VALIDATE_MOVE, new PostValidateMoveRoute(this.gson));

        post(SUBMIT_TURN, new PostSubmitTurnRoute(this.gson));

        post(CHECK_TURN, new PostCheckTurnRoute(this.gson));

        post(SPECTATOR_CHECK_TURN, new PostSpectatorCheckTurn(this.playerLobby, gson));

        post(BACK_UP, new PostBackupMoveRoute(this.gson));

        post(AI_GAME, new PostStartAIGame(this.templateEngine));

        get(SPECTATE_GAME, new GetSpectatorGame(this.playerLobby, this.templateEngine, gson));

        get(SPECTATOR_STOP_WATCHING, new GetSpectatorStopWatching(this.playerLobby, templateEngine, gson));

        post(HINT, new PostHintRoute(this.playerLobby, templateEngine, gson));
